    ...
    <>
        ...
        >
          ...
          >
            <Image
              src={lightbox.src}
              alt={lightbox.alt}
              className="object-contain max-h-[88vh] w-auto rounded-xl shadow-2xl"
              quality={95}
            />
          </div>
        </div>
      ...
      <main className="min-h-screen bg-background text-foreground">
        <div className="mx-auto max-w-[1060px] px-6 py-8 md:px-16">
          ...
          {/* Pipeline */}
          <section className="mb-16">
            <p className="mb-3 text-[11px] font-semibold tracking-[0.2em] text-secondary/70 uppercase">Overall pipeline</p>

            <div className="mt-4 rounded-2xl overflow-hidden border border-border bg-surface">
              <Image src={ingestion} alt="Pipeline of the dataprocess" className="w-full h-auto" quality={90} />
              <p className="text-[12px] text-muted/50 text-center py-3">Steps of the pipeline</p>
            </div>
          </section>

          {/* Ingestion layer */}
          <section className="mb-16">
            <p className="mb-3 text-[11px] font-semibold tracking-[0.2em] text-secondary/70 uppercase">Landing Layer</p>
            <div className="mb-6 flex flex-col md:flex-row items-start gap-6">
              <div className="flex-1 flex flex-col gap-4 text-[14px] leading-[1.8] text-justify text-foreground/55">
                <p>
                  This is the <strong className="text-foreground/75">first stage of the pipeline.</strong> Every day, the <strong className="text-foreground/75">Airflow DAG automatically uploads the next batch</strong> of PaySim CSV files into a <strong className="text-foreground/75">GCS bucket</strong> under{" "}
                  <code className="text-[12px] bg-surface border border-border rounded px-1.5 py-0.5">landing/incoming/paysim/</code>. No transformation happens at this point, the bucket is a staging area, holding the files until the processing step runs.
                </p>
                <p>
                  Once the batch lands, the DAG triggers the ingestion job: it picks up the file from{" "}
                  <code className="text-[12px] bg-surface border border-border rounded px-1.5 py-0.5">incoming/</code>, enforces the schema via PySpark, and <strong className="text-foreground/75">appends the rows directly to the Raw layer in Snowflake</strong>{" "}
                  (<code className="text-[12px] bg-surface border border-border rounded px-1.5 py-0.5">FRAUD_DB.RAW.raw_transactions</code>). After a successful write, the file is <strong className="text-foreground/75">moved to{" "}
                  <code className="text-[12px] bg-surface border border-border rounded px-1.5 py-0.5">landing/processed/</code></strong> so it is never re-ingested. Bad records are quarantined to{" "}
                  <code className="text-[12px] bg-surface border border-border rounded px-1.5 py-0.5">landing/error/</code> automatically via Spark&apos;s <code className="text-[12px] bg-surface border border-border rounded px-1.5 py-0.5">badRecordsPath</code> option, enabling full replay and auditing.
                </p>
              </div>
              <div className="w-full md:w-72 shrink-0">
                ...
                <p className="mt-2 text-[11px] text-muted/50 text-center">GCPDataReader — GCS to Snowflake via Spark</p>
              </div>
            </div>
            <div className="mt-4 rounded-2xl overflow-hidden border border-border bg-surface">
          ...
          <section className="mb-16">
            ...
            <p className="mb-6 text-[14px] leading-[1.8] text-justify text-foreground/55">
              The transformation layer is fully managed by <strong className="text-foreground/75">dbt Core</strong>. Once rows land in the Raw layer, dbt takes over and moves data through two distinct transformation stages before it is ready for analysis.
            </p>
            ...
            <p className="mb-6 text-[14px] leading-[1.8] text-justify text-foreground/55">
              The first stage is the <strong className="text-foreground/75">staging model</strong>. An incremental merge that runs on every DAG execution. It reads only new rows from <code className="text-[12px] bg-surface border border-border rounded px-1.5 py-0.5">RAW.raw_transactions</code> using a watermark on the ingestion timestamp, renames columns to a consistent snake_case convention, casts all types explicitly (amounts as <code className="text-[12px] bg-surface border border-border rounded px-1.5 py-0.5">FLOAT</code>, flags as <code className="text-[12px] bg-surface border border-border rounded px-1.5 py-0.5">BOOLEAN</code>), generates a <strong className="text-foreground/75">surrogate key</strong> from composite business attributes via <code className="text-[12px] bg-surface border border-border rounded px-1.5 py-0.5">dbt_utils.generate_surrogate_key</code>, computes balance delta fields, and deduplicates within the batch window. The watermark is set to <code className="text-[12px] bg-surface border border-border rounded px-1.5 py-0.5">max(ingested_at) − 5 minutes</code> to account for late-arriving records without reprocessing the full table.
            </p>
            ...
            <p className="mb-6 text-[14px] leading-[1.8] text-justify text-foreground/55">
              The second stage materialises the <strong className="text-foreground/75">Trusted layer</strong> as a star schema. Dimension tables cover accounts, transaction types, and dates. Fact tables include <code className="text-[12px] bg-surface border border-border rounded px-1.5 py-0.5">fct_fraud_events</code> (one row per fraudulent transaction with full context) and <code className="text-[12px] bg-surface border border-border rounded px-1.5 py-0.5">fct_balance_movements</code> (the complete transaction history). An aggregate table <code className="text-[12px] bg-surface border border-border rounded px-1.5 py-0.5">agg_account_balances</code> pre-computes total fraud exposure per account. <strong className="text-foreground/75">SCD Type 2 snapshots</strong> are applied to the account dimension to capture how account attributes change over time, enabling accurate point-in-time queries. All models are documented and tested inline — if any test fails, a dbt quality gate blocks the trusted layer from being populated that run.
            </p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div
                className="rounded-2xl overflow-hidden border border-border bg-surface cursor-zoom-in group"
                onClick={() => setLightbox({ src: fctBalance, alt: "fct_balance_movements fact table" })}
              >
                <Image src={fctBalance} alt="fct_balance_movements fact table" className="w-full h-auto transition-opacity group-hover:opacity-80" quality={90} />
                <p className="text-[12px] text-muted/50 text-center py-3">fct_balance_movements — full transaction history</p>
              </div>
              <div
                className="rounded-2xl overflow-hidden border border-border bg-surface cursor-zoom-in group"
                onClick={() => setLightbox({ src: aggBalances, alt: "agg_account_balances aggregate table" })}
              >
                <Image src={aggBalances} alt="agg_account_balances aggregate table" className="w-full h-auto transition-opacity group-hover:opacity-80" quality={90} />
          ...
          <section className="mb-16">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4"
              onClick={() => setLightbox({ src: dag, alt: "Airflow DAG graph view" })}
              >
              <div className="rounded-2xl overflow-hidden border border-border bg-surface">
                <Image src={dag} alt="Airflow DAG graph view" className="w-full h-auto transition-opacity group-hover:opacity-80" quality={90} />
                <p className="text-[12px] text-muted/50 text-center py-3">paysim_batch_ingestion DAG graph</p>
              </div>
              <div
                className="rounded-2xl overflow-hidden border border-border bg-surface cursor-zoom-in group"
                onClick={() => setLightbox({ src: airflowUi, alt: "Airflow UI run history" })}
              >
                <Image src={airflowUi} alt="Airflow UI run history" className="w-full h-auto transition-opacity group-hover:opacity-80" quality={90} />
          ...
          <section className="mb-16">
            ...
            <div className="mb-6 flex flex-col md:flex-row items-start gap-6">
              <div className="flex-1 flex flex-col gap-4 text-[14px] leading-[1.8] text-justify text-foreground/55">
                <p>
                  All cloud and warehouse resources are <strong className="text-foreground/75">fully managed as code using Terraform</strong>. Nothing is created manually: every bucket, service account, IAM binding, and Snowflake object is declared in version-controlled configuration files and applied via <code className="text-[12px] bg-surface border border-border rounded px-1.5 py-0.5">terraform apply</code>.
                </p>
                <p>
                  On the <strong className="text-foreground/75">GCP side</strong>, Terraform provisions the GCS bucket with structured folder prefixes (<code className="text-[12px] bg-surface border border-border rounded px-1.5 py-0.5">incoming/</code>, <code className="text-[12px] bg-surface border border-border rounded px-1.5 py-0.5">processed/</code>, <code className="text-[12px] bg-surface border border-border rounded px-1.5 py-0.5">error/</code>), lifecycle rules to auto-delete old processed files, and a dedicated GCP service account with least-privilege IAM bindings — only the permissions the pipeline actually needs.
                </p>
                <p>
                  On the <strong className="text-foreground/75">Snowflake side</strong>, Terraform creates the warehouse, the <code className="text-[12px] bg-surface border border-border rounded px-1.5 py-0.5">FRAUD_DB</code> database, and the three schemas (<code className="text-[12px] bg-surface border border-border rounded px-1.5 py-0.5">RAW</code>, <code className="text-[12px] bg-surface border border-border rounded px-1.5 py-0.5">STAGING</code>, <code className="text-[12px] bg-surface border border-border rounded px-1.5 py-0.5">TRUSTED</code>) with separate roles and access grants per layer. This ensures that transformation jobs can only write to the layer they own and never touch upstream data.
                </p>
              </div>
              <div className="w-full md:w-80 shrink-0">
                <div
                  className="rounded-2xl overflow-hidden border border-border bg-surface cursor-zoom-in group"
                  onClick={() => setLightbox({ src: snowflake, alt: "Snowflake schema setup" })}
                >
                  <Image src={snowflake} alt="Snowflake schema setup" className="w-full h-auto transition-opacity group-hover:opacity-80" quality={90} />
            ...
            <div
              className="rounded-2xl overflow-hidden border border-border bg-surface cursor-zoom-in group"
              onClick={() => setLightbox({ src: terraform, alt: "Terraform IaC setup" })}
            >
              <Image src={terraform} alt="Terraform IaC setup" className="w-full h-auto transition-opacity group-hover:opacity-80" quality={90} />
              <p className="text-[12px] text-muted/50 text-center py-3">Terraform — GCS bucket, lifecycle rules & IAM provisioning</p>
            </div>
          </section>
